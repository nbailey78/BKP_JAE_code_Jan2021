...
/* Experiment 2B: Two observed factors - Non-Gaussian errors (Power functions) */
/* Figure 1- main paper; Figures S8-S12, S14 - appendix */

_dxmiss=0/0;
_optnorm=2; /* =0 normal errors; =1 t-distributed errors; =2 chi-squared errors */
...
cov_f = { 1 0.3, 0.3 1 }; /* Covarianve structure of factors */
...
grid_alpha =seqa(0.7,0.05,7); /* true values of alpha */
ngp_a1=rows(grid_alpha);
if factor==1;
grid_alpha2=0;
elseif factor==2; 
grid_alpha2=grid_alpha;
endif;
ngp_a2=rows(grid_alpha2);
...
sto_bias_o1=zeros(repl,ngp_a1*ngp_a2);
sto_mse_o1=zeros(repl,ngp_a1*ngp_a2);    
sto_size_fo1=zeros(repl,ngp_a1*ngp_a2);
    
sto_bias_o2=zeros(repl,ngp_a1*ngp_a2);
sto_mse_o2=zeros(repl,ngp_a1*ngp_a2);   
sto_size_fo2=zeros(repl,ngp_a1*ngp_a2);
...
for m(1,rows(grid_alpha),1);  /* loop of strength of 1st factor */
power_f11={};
power_f22={};   
 for q(1,rows(grid_alpha2),1);  /* loop of strength of 2nd factor */
  nin~tit~o~grid_alpha[m,1]~grid_alpha2[q,1];
...
for r(1,rows(ap),1);  
 ...
 /* Estimation of alpha */
 /* Observed factor */
 {rej,rej,power_fo}=a_d(x0,xp,f,p_val,delta,p_test,a0,ap[r,.],0); 
...
if factor==1;
 ...
 elseif factor==2;
  power_f11=power_f11|(powerp1);   
  power_f22=power_f22|(powerp2);    
endif;
endfor;

if factor==1;
  s_powerp1=reshape(power_ra1,1,ngp_a1*rows(ap));
elseif factor==2;
  s_powerp1=reshape(power_f11,1,ngp_a2*rows(ap));  
  m_power_ra1=m_power_ra1|(s_powerp1);
  s_powerp2=reshape(power_f22,1,ngp_a2*rows(ap));  
  m_power_ra2=m_power_ra2|(s_powerp2);  
endif;
endfor;

if factor==1;
 m_power_o1=m_power_o1|(s_powerp1);
elseif factor==2; 
 m_powerp1=reshape(m_power_ra1,1,ngp_a1*ngp_a2*rows(ap));  
 m_power_o1=m_power_o1|(m_powerp1);
 m_powerp2=reshape(m_power_ra2,1,ngp_a1*ngp_a2*rows(ap));  
