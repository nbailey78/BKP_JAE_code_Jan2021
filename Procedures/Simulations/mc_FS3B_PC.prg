...
/* Experiment 3B: Two unobserved factors - non-Gaussian errors (Bias, RMSE - strongest factor PC) */
/* Tables S18-S19- appendix */
...
a_size=0.1; /* p-value for JAE (2016) estimator */
...
grid_alpha1=seqa(0.85,0.05,4).*.ones(4,1); /* strength of first factor */
grid_pr2={0.51,0.75,0.95,1};
grid_alpha2=ones(4,1).*.grid_pr2; /* stength of second factor */
if factor==1;
grid_alpha=seqa(0.85,0.05,4);
elseif factor==2; 
...
x=ones(t,n)*c_v+f*gamm+eps;
 ...
 /* Estimation of alpha */
 /*  PCA factors */
{a_hato,size_fo,rej,rej}=a_d(x,f,p_val,delta,p_test,a0,a1p,a1m,2);
...
if factor==1;
 ...
 elseif factor==2;   
  sto_bias_o1[o,m]=a_hato[1,1]-maxc(a0');
  sto_mse_o1[o,m]=(a_hato[1,1]-maxc(a0'))^2;  
...
m_size_fo1=(sumc(sto_size_fo1)'/repl)*100;
...
local n,t,x_ave,pmax,pc,rhsv,coef,res,sig_vec,t_stat,cv,di,d_bar,a_hat,ze_nf,psi_nf,z_af,z_a1pf,z_a1mf,ze_ninf,psi_ninf,ze_ninf1p,psi_ninf1p,ze_ninf1m,psi_ninf1m,z_ainf,z_a1pinf,z_a1minf,size_f,power_pf,power_mf,
...
if opt==0; /* use observed f */  
 ...
 x_ave=meanc(x');  
 a0=maxc(a0');    
elseif opt==2; /* use PCA simultaneously */
 pmax=1; 
 {pc}=getpc1(x,pmax);    
 a0=maxc(a0');     
 x_ave=pc[.,cols(pc)];   
