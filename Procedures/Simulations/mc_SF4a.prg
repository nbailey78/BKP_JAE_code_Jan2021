...
/* Experiment 4a: Observed misspecified single factor - Gaussian errors (Bias, RMSE - corr factors */
/* Table S20a - appendix */
_dxmiss=0/0;
...
grid_alpha1=ones(1,1).*.ones(7,1); /* strength of first factor */
grid_alpha2=ones(1,1).*.seqa(0.7,0.05,7); /* stength of second factor */
if factor==1;
grid_alpha=1;
elseif factor==2; 
...
x1m=ones(t,n)*c_v+f*gamm1m+eps; 
 ...
 /* Estimation of alpha */
 /* Observed factor */
{a_hato,size_fo,rej,rej}=a_d(x,f[.,1],p_val,delta,p_test,a0,a1p,a1m,0);
/*{rej,rej,powerp_fo,rej}=a_d(x1p,f[.,1],p_val,delta,p_test,a0,a1p,a1m,0); 
{rej,rej,rej,powerm_fo}=a_d(x1m,f[.,1],p_val,delta,p_test,a0,a1p,a1m,0); */
